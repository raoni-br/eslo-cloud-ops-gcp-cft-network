...
data "google_projects" "dns_hub" {
  filter = "parent.id:${split("/", data.google_active_folder.shared.name)[1]} labels.application_name=org-dns-hub lifecycleState=ACTIVE"
}
...
module "dns_hub_vpc" {
  ...
  network_name                           = "vpc-s-dns-hub"
  shared_vpc_host                        = "false"
  delete_default_internet_gateway_routes = "true"

  subnets = [{
    subnet_name           = "sb-s-dns-hub-${var.default_region1}"
    ...
    subnet_name           = "sb-s-dns-hub-${var.default_region2}"
  ...
  routes = [{
    name              = "rt-s-dns-hub-1000-all-default-private-api"
...
module "dns_hub_region1_router1" {
  ...
  version = "~> 0.3.0"
  name    = "cr-s-dns-hub-${var.default_region1}-cr1"
  project = local.dns_hub_project_id
...
module "dns_hub_region1_router2" {
  source  = "terraform-google-modules/cloud-router/google"
  version = "~> 0.3.0"
  name    = "cr-s-dns-hub-${var.default_region1}-cr2"
  project = local.dns_hub_project_id
  network = module.dns_hub_vpc.network_name
  region  = var.default_region1
  bgp = {
...
module "dns_hub_region2_router1" {
  source  = "terraform-google-modules/cloud-router/google"
  version = "~> 0.3.0"
  name    = "cr-s-dns-hub-${var.default_region2}-cr3"
  project = local.dns_hub_project_id
  network = module.dns_hub_vpc.network_name
...
module "dns_hub_region2_router2" {
  source  = "terraform-google-modules/cloud-router/google"
  version = "~> 0.3.0"
  name    = "cr-s-dns-hub-${var.default_region2}-cr4"
  project = local.dns_hub_project_id
  network = module.dns_hub_vpc.network_name
  region  = var.default_region2
  bgp = {
