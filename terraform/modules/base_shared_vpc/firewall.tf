...
// TODO: ALLOWING TRAFFIC TO GOOGLE APIS FROM ANYWHERE
// Allow traffic from VPC Connector to Google APIs 
resource "google_compute_firewall" "allow_vpc_connector_private_api_egress" {
  count     = var.serverless_vpc_connector != null ? 1 : 0
  name      = "fw-${var.environment_code}-shared-base-65534-e-a-allow-vpc-conn-apis-all-tcp-443"
  network   = module.main.network_name
  project   = var.project_id
  direction = "EGRESS"
  priority  = 65534
  ...
  # source_ranges = [var.serverless_vpc_connector.ip_cidr_range]
}


// Allow traffic from VPC Connector to Shared VPC
resource "google_compute_firewall" "allow_vpc_connector_private_service_egress" {
  count     = var.serverless_vpc_connector != null ? 1 : 0
  name      = "fw-${var.environment_code}-shared-base-65534-e-a-allow-vpc-conn-pvt-serv-all-tcp"
  network   = module.main.network_name
  project   = var.project_id
  direction = "EGRESS"
  priority  = 65534
  ...
  destination_ranges = [var.private_service_cidr]
